...
bool AABB(float x, float y, float w, float h, float x2, float y2, float w2,
          float h2);

bool AABB(Asset_Ase *a, Asset_Ase *b, float x, float y, float x2, float y2);

bool AABB_Movement(float x, float y, float w, float h, Asset_Ase *a, float x2,
                   float y2);

bool AABB_Movement(Rect rect, Asset_Ase *a, float x2, float y2);

bool AABB_Collision(float x, float y, float w, float h, Asset_Ase *a, float x2,
                    float y2);
...
template <typename A, typename B> bool PolygonPoint(v2 *vertices, A px, B py);
...
float AngleBetweenTwoPoints(int target_x, int target_y, int origin_x,
                            int origin_y);
...
bool AABB(Asset_Ase *a, Asset_Ase *b, float x, float y, float x2, float y2) {
  return x + a->collision_box->x + a->collision_box->w >
             x2 + b->collision_box->x &&
         x + a->collision_box->x <
             x2 + b->collision_box->x + b->collision_box->w &&
         y + a->collision_box->y + a->collision_box->h >
             y2 + b->collision_box->y &&
         y + a->collision_box->y <
             y2 + b->collision_box->y + b->collision_box->h;
}

bool AABB_Movement(float x, float y, float w, float h, Asset_Ase *a, float x2,
                   float y2) {
  return x + w > x2 + a->movement_box->x &&
         x < x2 + a->movement_box->x + a->movement_box->w &&
         y + h > y2 + a->movement_box->y &&
         y < y2 + a->movement_box->y + a->movement_box->h;
}
...
bool AABB_Collision(float x, float y, float w, float h, Asset_Ase *a, float x2,
                    float y2) {
  return x + w > x2 + a->collision_box->x &&
         x < x2 + a->collision_box->x + a->collision_box->w &&
         y + h > y2 + a->collision_box->y &&
         y < y2 + a->collision_box->y + a->collision_box->h;
}
